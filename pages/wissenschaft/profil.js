  ...
  return (
    <>
      ...
      <Text className={style.mtDefault}>
        In der ersten Phase meines wissenschaftlichen Arbeitens war ich noch stark von älteren Spielarten musikwissenschaftlicher Hermeneutik beeinflusst. Seit 2005 hat mich dann verstärkt das Denken anderer Disziplinen wie der Informatik und der Soziologie interessiert. Wichtig war für mich insbesondere, die eigene Forschungsperspektive beständig zu reflektieren sowie die Erkenntnis, dass die Erforschung psychischer Vorgänge (Stichwort: <i>Komponistenintention</i>) immer auf nicht verifizierbare Spekulationen angewiesen ist. Für meine eigenen Arbeiten haben solche Fragestellungen daher ihre Bedeutung verloren. In meiner Monografie (Dissertation) zum jungen Mozart habe ich mich dann intensiv mit methodischen Fragen der musikalischen Analyse auseinandergesetzt und z.B. für die Werke des Wunderkindes eine <b><i>synoptische Notenanalyse</i></b> vorgeschlagen:  
      </Text>
      <UnorderedList className={style.mtDefault}>
        <ListItem className={style.mlDefault}><Link href='https://github.com/musikisum/mozartforschung.de/blob/main/src/downloads/kaiser_notenbuechermozarts-kassel-2007.sm.pdf' isExternal><i>Die Notenbücher der Mozarts als Grundlage der Analyse von W. A. Mozarts Kompositionen 1761−1767</i> <ExternalLinkIcon /></Link>, Kassel 2007.</ListItem>
      </UnorderedList>
      <Text className={style.mtDefault}>
        In dem Kapitel »Musikalische Modelle« dieser Monografie findet sich auch ein erster Versuch, das Design musiktheoretischer Modelle an die <b>Objektorientierung der Programmierung</b> anzulehnen. In diesem Sinne lassen sich Modelle als Abstraktionen von Objekten verstehen, die über Klassen oder Prototypen repräsentiert werden, welche durch Eigenschaften und ein Verhalten charakterisiert sind. Sowohl für den allgemeinen Modellbegriff als auch für die Programmierung gilt, dass umso kleiner und ärmer der Inhalt wird, desto abstrakter das Modell ist (und umgekehrt). Meine Bemühungen der Übertragung habe ich in dem folgenden Beitrag dargelegt:
      </Text>
      <UnorderedList className={style.mtDefault}>
      ...
      <UnorderedList className={style.mtDefault}>
        <ListItem className={style.mlDefault}>
          <b>Theorie der Sonatenform im 18. und frühen 19. Jahrhundert.</b> Seit meiner Dissertation zu der Musik Mozarts habe ich mich gefragt, wie eine aktuelle Sonatentheorie aussehen müsste, die sich zur Analyse eines größeren Repertoires eignet. Dabei lasse ich mich durch historische Ideen inspirieren (z.B. Kadenzen frei nach H. Chr. Koch, Regola dell&rsquo;ottava usw.) und kombiniere die Gedanken mit systematischen Überlegungen zur musikalischen Analyse (z.B. der Bedeutung von Satzmodellen und forschungsgelenkten Modellbildungen).
        </ListItem>
      </UnorderedList>
      <UnorderedList className={style.mtDefault}>
        <ListItem className={style.mlDefault}>
